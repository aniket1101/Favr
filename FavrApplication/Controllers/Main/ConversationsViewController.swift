    ...
    @objc private func didTapSearchButton() {
        ...
        vc.completion = { [weak self] result in
            ...
            if let targetConversation = currentConversations.first(where: {
                ...
                let reference = Database.database().reference().child(DatabaseManager.safeEmail(emailAddress: result.email))
                reference.child("name").observeSingleEvent(of: .value, with: {
                    snapshot in
                    let newName = snapshot.value as? String
                    print("New name is \(newName ?? "New name not found")")
                    vc.title = newName
                })
            }
            else {
    ...
    private func createNewConversation(result: SearchResult) {
        ...
        DatabaseManager.shared.conversationExists(with: email, completion: { [weak self] result in
            ...
            case .success(let conversationId):
                ...
                vc.navigationItem.largeTitleDisplayMode = .never
                strongSelf.navigationController?.pushViewController(vc, animated: true)
                let reference = Database.database().reference().child(email)
                reference.child("name").observeSingleEvent(of: .value, with: {
                    snapshot in
                    let newName = snapshot.value as? String
                    vc.title = newName
